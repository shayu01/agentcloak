...
Complete catalog of every daemon route, with its CLI and MCP bindings. Generated from the FastAPI OpenAPI spec — the source of truth lives in `daemon/models.py` (Pydantic request schemas) and `daemon/routes.py` (route decorators).
