...
### Phase 6: API-First Spell Platform

> 核心理念：**API-first, UI-fallback**（参考 neo 的双通道设计）。spell 应该能脱离浏览器独立运行，只在必要时才开浏览器。

#### 6a: Spell 独立运行（不依赖浏览器）

当前所有非 PUBLIC 的 spell 都需要浏览器（从浏览器提取 cookies/UA）。目标是让 spell 能用持久化的 credentials 独立调 API。
...
#### 6b: Workflow Discover + OpenAPI 导出

从 capture 自动发现操作流程，导出为可复用的 spell 或 OpenAPI spec。
...
- [ ] drag & drop、剪贴板
- [ ] multi-session（同时管理多个站点的独立 context）
...
### API-First Spell Strategy (D22, new)
